...
g = svg.append('g').attr('transform',`translate(${diameter/2},${diameter/2})`), //translate to 2, 2 location from default 0,0 location, very minor move
...
d3.json('flare.json',(error,root)=>{
  ...
  let circle = g.selectAll("circle")
                .data(nodes)
                .enter().append('circle')
                .attr('class',d=>d.parent ? d.children ? 'node' : 'node node node--leaf' : 'node node--root')
